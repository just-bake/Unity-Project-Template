...
namespace Kamgam.SettingsGenerator
{
    ...
    public class SettingResolverEditor : Editor
    {
...
#else
            var isInPrefabStage = UnityEditor.Experimental.SceneManagement.PrefabStageUtility.GetCurrentPrefabStage() != null;
#endif
            
            // Auto select if settings provider is null
            // Only do this if the object is a prefab instance.
            // Sadly PrefabUtility.IsPartOfPrefabContents() is not available in Unity 2020 and 2021, thus we have to fall back on prefab stage check.
            if (resolver.SettingsProvider == null && (AlwaysAutoAssignProvider || (!isInPrefabStage && !PrefabUtility.IsPartOfPrefabAsset(resolver.gameObject))))
            {
        ...
        public override void OnInspectorGUI()
        {
            ...
            bool hasProvider = resolver.SettingsProvider != null && resolver.SettingsProvider.SettingsAsset != null;
            if (GUILayout.Button(new GUIContent(" ➔ ", "Click here to jump to the settings list."), GUILayout.MaxWidth(30)))
            {
                ...
                {
                    CreateSettingUGUIWindow.ShowMenuCreator();
                }
            }
