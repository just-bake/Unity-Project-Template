        ...
        public static void FromJson(string json, Settings settings)
        {
            ...
            foreach (var setting in fields)
            {
                foreach (var settingData in data.Fields)
                {
                    ...
                    {
                        // Fix for setting IDs that exist in the saved data with a different type (those will be ignored and will vanish in the next save cycle).
                        // Usually this happens if a user switches from KeyCombination bindings to the new input bindings (one is of type KeyCombination, the other of type String).
                        if (settingData.Type != setting.GetDataType())
                        {
                            Debug.LogWarning("SGSettings: Data type conflict for path '" + settingData.ID + "'. " +
                                                 "Saved setting type is '" + settingData.Type + "' but setting type is '" + setting.GetDataType() + "'. " +
                                                 "Maybe you have two settings with the same ID '" + setting.GetID() + "' OR you have some settings saved with that type? " +
                                                 "Will skip loading data from type '" + settingData.Type + "'. Solutions: Check for duplicate IDs and delete any saved settings data. Then try again.");
                            continue;
                        }
            ...
            // Create settings for data fields that are not in the settings.
            // These might be connected later (via script).
            foreach (var settingData in data.Fields)
            {
                bool found = false;
                foreach (var setting in fields)
