...
// Notice: The RefreshRate class has been added in 2021_2 (yes 2021) BUT it is used in Screen.SetResolution only in 2022.2 (2022, not 2021)
            ...
            yield return null; // <- not sure if needed, More testing needed.

            OnComplete?.Invoke(tRequestedResolution, tRequestedFullScreen, tRequestedFullScreenMode);
        }
        ...
        public static Resolution GetCurrentResolution()
        {
            ...
            {
                // In windowed mode Screen.currentResolution would return the monitor resolution instead
                // of the native window size, which is not what we want. Thus we create our own based on
                // the window size.
