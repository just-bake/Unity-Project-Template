...
namespace Kamgam.SettingsGenerator
{
    public class MonitorConnection : ConnectionWithOptions<string>, IConnectionWithSettingsAccess
    {
        ...
        /// <summary>
        /// Enable this if you are having difficulties with monitor settings not being set at the start.<br />
        /// This will also be set to TRUE automatically if two monitors with the exact same name and resolution are detected.
        /// </summary>
...
#pragma warning restore CS0067
        ...
        /// Is disabled by default due to backwards compatibility.
        /// </summary>
        ...
        public override List<string> GetOptionLabels()
        {
            if (_labels.IsNullOrEmpty())
            {
                ...
                int sameMonitorCounter = 1;
                foreach (var info in displays)
                {
                    string name = info.name + " (" + info.width + "x" + info.height + ")";
                    
                    // Fix for two identical monitors.
                    int sameMonitorNameIndex = _labels.IndexOf(name);
                    if (sameMonitorNameIndex >= 0)
                    {
                        _labels[sameMonitorNameIndex] += " " + sameMonitorCounter;
                        sameMonitorCounter++;
                        name += " " + sameMonitorCounter;
...
#if UNITY_EDITOR
            if (SettingsGeneratorSettings.GetOrCreate().ShowEditorInfoLogs)
            {
                Logger.LogMessage("Setting the monitor has no effect in the Editor. Please try in a build. - " + SettingsGeneratorSettings.ShowEditorInfoLogsHint);
            }
#endif
        }
        ...
        void moveToMonitor(int index)
        {
            try
            {
                ...
                {
                    // Remember current resolution and window size.
                    _windowSizeBeforeMonitorChange = new Vector2Int(Screen.width, Screen.height);
                    _resolutionBeforeMonitorChange = null;
                    if (TryToPreserveResolutionOnMonitorChange)
                    {
                        _resolutionBeforeMonitorChange = Screen.currentResolution;
                    }

                    // Move
                    _moveOperation = Screen.MoveMainWindowTo(display, Vector2Int.zero);
        ...
        private static List<ResolutionConnection> s_tmpResolutionConnections = new List<ResolutionConnection>();
        ...
        async void waitForMonitorSwitchToComplete()
        {
            ...
            {
                // Refresh resolution and refresh-rate connections and resolvers
                var settings = SettingsProvider.LastUsedSettingsProvider.Settings;
                
                // Resolution
                settings.GetSettingsWithConnectionByType<SettingOption, ResolutionConnection>(s_tmpOptionSettingsList);
                foreach (var setting in s_tmpOptionSettingsList)
                {
                    if (setting.HasConnection())
                    {
                        var connection = setting.GetConnectionInterface() as ResolutionConnection;
                        var cache = connection.CacheResolutions;
                        connection.CacheResolutions = false; // Force disable cache.
                        connection.RefreshOptionLabels();
                        connection.CacheResolutions = cache; // Restore cache.
            ...
            if (TryToPreserveResolutionOnMonitorChange && _resolutionBeforeMonitorChange.HasValue)
            {
                // The desired with x height is only the starting point for finding the closest resolution on the new monitor. If 0/0 the nothing will be done. 
                int desiredWidth = 0;
                int desiredHeight = 0;
                int desiredRefreshRate = 0;
                if (Screen.fullScreenMode == FullScreenMode.Windowed)
                {
                    // In windowed mode do nothing except if the window is bigger than the new monitor's resolution in which case reduce the window size.
                    if (_windowSizeBeforeMonitorChange.x > Screen.currentResolution.width ||
                        _windowSizeBeforeMonitorChange.y > Screen.currentResolution.height)
                    {
                        desiredWidth = Screen.currentResolution.width;
                        desiredHeight = Screen.currentResolution.height;
                        desiredRefreshRate = ResolutionConnection.GetRoundedRefreshRate(Screen.currentResolution);
                    }
                }
                else
                {
                    // Only do this if in fullscreen-ish mode
                    desiredWidth = _resolutionBeforeMonitorChange.Value.width;
                    desiredHeight = _resolutionBeforeMonitorChange.Value.height;
                    desiredRefreshRate = ResolutionConnection.GetRoundedRefreshRate(_resolutionBeforeMonitorChange.Value);
                }
                ...
                if (desiredWidth > 0 && desiredHeight > 0 && desiredRefreshRate > 0)
                {
                    ...
                    if (settings != null)
                    {
                        var connections = settings.GetConnectionsByType<ResolutionConnection>(s_tmpResolutionConnections);
                        if (connections.Count == 0)
                        {
                            // Create tmp ResolutionConnection if no setting with a ResolutionConnection exists.
                            if (s_tmpResolutionConnection == null)
                            {
                                s_tmpResolutionConnection = new ResolutionConnection();
                                s_tmpResolutionConnection.RefreshRateResolversAfterCompletion = true;
                            }

                            updateResolutionConnectionToClosetsResolution(s_tmpResolutionConnection, settings, desiredWidth, desiredHeight, desiredRefreshRate);
                        }
                        else
                        {
                            // Update all ResolutionConnections (usually just one).
                            foreach (var connection in connections)
                            {
                                updateResolutionConnectionToClosetsResolution(connection, settings, desiredWidth, desiredHeight, desiredRefreshRate);
                            }
                        }
                    }
                }
                // Reset tmp values
                s_tmpResolutionConnections.Clear();
                _resolutionBeforeMonitorChange = null;
                _windowSizeBeforeMonitorChange = Vector2Int.zero;
            }
        ...
        protected void updateResolutionConnectionToClosetsResolution(ResolutionConnection connection, Settings settings, int width, int height, int refreshRate)
        {
            ...
            var resolutionIndex = connection.FindClosestResolutionIndex(width, height, refreshRate);
            if (resolutionIndex >= 0)
            {
                // v- this will trigger a pullFromConnection in onComplete which updates the setting's resolvers.
                connection.Set(resolutionIndex);
                // However we still trigger an update on the setting here (if a setting exists) because
                // if RefreshRateResolversAfterCompletion = false then it would not trigger a resolver update.
                var resolutionSettings = settings.GetSettingsWithConnection(connection);
                resolutionSettings.PullFromConnection();
                resolutionSettings.RefreshRegisteredResolvers(settings);
            }
        }
#endif
