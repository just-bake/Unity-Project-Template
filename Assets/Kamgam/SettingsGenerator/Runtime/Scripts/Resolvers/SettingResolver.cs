...
namespace Kamgam.SettingsGenerator
{
    ...
    public abstract class SettingResolver : MonoBehaviour, ISettingResolver
    {
        ...
        public SettingsProvider GetProvider() => SettingsProvider;
        public SettingsProvider SetProvider(SettingsProvider provider) => SettingsProvider = provider;
        ...
        public virtual void OnEnable()
        {
            ...
            {
                SettingsProvider.Settings.ActiveResolverCount++;
            }
        }
        ...
        public virtual void OnDisable()
        {
            if (SettingsProvider != null && SettingsProvider.HasSettings())
            {
                ...
                // Check if all resolvers have been disabled again
                if (SettingsProvider.Settings.ActiveResolverCount <= 0)
                {
                    SettingsProvider.OnAllResolversDeactivated(_isQuitting);
                }
            }
        }
        ...
        public static System.Collections.Generic.List<ISettingResolver> FindResolversInLoadedScenes(bool includeInactive = true)
        {
            var resolvers = new System.Collections.Generic.List<ISettingResolver>();
            ...
            for (int i = 0; i < SceneManager.sceneCount; i++)
            {
                ...
                {
                    var tmp = root.GetComponentsInChildren<ISettingResolver>(includeInactive);
                    foreach (var resolver in tmp)
                    {
                        if( resolver != null)
        ...
        public static System.Collections.Generic.List<ISettingResolver> EditorFindResolversInLoadedScenes(bool includeInactive = true)
        {
            var resolvers = new System.Collections.Generic.List<ISettingResolver>();
