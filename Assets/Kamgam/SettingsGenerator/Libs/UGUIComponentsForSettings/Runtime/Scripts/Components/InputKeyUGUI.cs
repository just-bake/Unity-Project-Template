    ...
    public class InputKeyUGUI : MonoBehaviour
    {
        ...
        public void Update()
        {
            ...
            if (IsActive && !waitForKeyRelease)
            {
                ...
                bool mouseUsed = InputUtils.MouseUp() || InputUtils.MouseWheelUsed();
                
#if ENABLE_INPUT_SYSTEM
                // TODO: Investigate if InputSystem is used then the wheel has to be turned twice because _aKeyWasPressedWhileActive
                // will only be set to true AFTER this check. Should investigate why. Right now this is fixed by adding
                // the "InputUtils.MouseWheelUsed()" if below.
                if (InputUtils.MouseWheelUsed())
                {
                    _aKeyWasPressedWhileActive = true;
                    _keyWhileActive = InputUtils.GetUniversalKeyDown(excludeModifierKeys: true, excludeMouseButtons: false);
                }
#endif
                
                if ((_aKeyWasPressedWhileActive) && (keyPressStopped || mouseUsed))
                {
                    SetActive(false);

                    // Don't set key if mouse was pressed yet mouse is ignored.
                    if (!mouseUsed || AllowMouseButtons)
                    {
