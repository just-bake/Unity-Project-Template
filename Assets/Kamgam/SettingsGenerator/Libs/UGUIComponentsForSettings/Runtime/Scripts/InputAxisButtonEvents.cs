...
namespace Kamgam.UGUIComponentsForSettings
{
    public class InputAxisButtonEvents : MonoBehaviour
    {
        ...
        public const string MouseWheelAxis = "Mouse ScrollWheel"; // Mouse Scroll Wheel
        ...
        float _previousDownMouseWheel = 0f;
        ...
        float _previousUpMouseWheel = 0f;

        List<UniversalKeyCode> downButtons = new List<UniversalKeyCode>(5);
        public static int DownButtonCount => Instance == null ? 0 : Instance.downButtons.Count;

        List<UniversalKeyCode> upButtons = new List<UniversalKeyCode>(5);
        public static int UpButtonCount => Instance == null ? 0 : Instance.upButtons.Count;

        List<UniversalKeyCode> pressedButtons = new List<UniversalKeyCode>(5);
        public static int PressedButtonCount => Instance == null ? 0 : Instance.pressedButtons.Count;
        ...
        protected void updateStateIfNeeded()
        {
            ...
            detectDown(MouseWheelAxis, UniversalKeyCode.MouseWheelDown, UniversalKeyCode.MouseWheelUp, ref _previousDownMouseWheel, 0.01f);

            foreach (var downBtn in downButtons)
            {
                if (!pressedButtons.Contains(downBtn))
                    pressedButtons.Add(downBtn);
            }
            ...
            detectUp(MouseWheelAxis, UniversalKeyCode.MouseWheelDown, UniversalKeyCode.MouseWheelUp, ref _previousUpMouseWheel, 0.01f);

            foreach (var upBtn in upButtons)
            {
                pressedButtons.Remove(upBtn);
            }
        }
        ...
        public static bool GetButton(UniversalKeyCode button)
        {
            ...
            Instance.updateStateIfNeeded();
            return Instance.pressedButtons.Contains(button);
        }

        public static UniversalKeyCode GetButtonAt(int index)
        {
            if (Instance == null)
                return UniversalKeyCode.None;

            Instance.updateStateIfNeeded();
            return Instance.pressedButtons[index];
        }

        public static UniversalKeyCode GetDownButtonAt(int index)
        {
            if (Instance == null)
                return UniversalKeyCode.None;

            Instance.updateStateIfNeeded();
            return Instance.downButtons[index];
        }

        public static UniversalKeyCode GetUpButtonAt(int index)
        {
            if (Instance == null)
                return UniversalKeyCode.None;

            Instance.updateStateIfNeeded();
            return Instance.upButtons[index];
        }
