...
namespace Kamgam.UGUIComponentsForSettings
{
    ...
    public class SliderUGUIEditor : Editor
    {
        ...
        override public void OnInspectorGUI()
        {
            ...
            var oldMinValue = slider.MinValue;
            slider.MinValue = EditorGUILayout.FloatField("Min Value:", slider.MinValue);
            if (oldMinValue != slider.MinValue)
            {
                markAsChangedIfEditing();
            }

            var oldMaxValue = slider.MaxValue;
            slider.MaxValue = EditorGUILayout.FloatField("Max Value:", slider.MaxValue);
            if (oldMaxValue != slider.MaxValue)
            {
                markAsChangedIfEditing();
            }

            var oldWholeNumbersValue = slider.WholeNumbers;
            slider.WholeNumbers = EditorGUILayout.Toggle("Whole Numbers:", slider.WholeNumbers);
            if (oldWholeNumbersValue != slider.WholeNumbers)
            {
                markAsChangedIfEditing();
            }
